/**
 * VisualizadorEquipmentTable.jsx - Tabla paginada de equipos para Visualizador
 *
 ...
 * - Filtros por columna
 * - Paginacion
 * - Click en fila abre vista de detalle (solo lectura)
 *
 * @version 1.0.0
 */
...
/**
 * Componente VisualizadorEquipmentTable
 */
export const VisualizadorEquipmentTable = ({
  equipment = [],
  plantName,
  plantLocation,
  onSelectEquipment,
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                ...
                {activeFiltersCount > 0 && (
                  <span className="bg-purple-400 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">
                    {activeFiltersCount}
                  </span>
                )}
              </button>
      ...
      {showFilters && (
        <div className="px-4 sm:px-6 py-3 bg-purple-50 border-b border-purple-200 flex flex-col gap-3">
          {/* Datos faltantes */}
          <div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-2">
              {MISSING_DATA_FILTERS.map(({ key, label }) => (
                <label key={key} className="flex items-center gap-2 cursor-pointer group">
                  ...
                  />
                  <span className={`text-sm ${missingDataFilters[key] ? "text-purple-700 font-medium" : "text-gray-700"} group-hover:text-gray-900`}>
                    {label}
                  </span>
                </label>
              ))}
              ...
              {[
                ...
                <label key={key} className="flex items-center gap-2 cursor-pointer group">
                  <input
                    ...
                    className="w-4 h-4 rounded border-gray-300 cursor-pointer accent-purple-500"
                  />
                  <span className={`text-sm ${originFilters[key] ? "text-purple-700 font-medium" : "text-gray-700"} group-hover:text-gray-900`}>
                    {label}
                  </span>
                </label>
              ))}
            ...
            {isLoading ? (
              ...
              paginatedEquipment.map((equip, index) => {
                ...
                return (
                  <tr
                    key={equip.id}
                    className="hover:bg-purple-50 transition-colors odd:bg-white even:bg-gray-50 cursor-pointer focus:outline-none focus:bg-purple-100 focus:ring-2 focus:ring-inset focus:ring-purple-400"
                    ...
                    tabIndex={0}
                    role="button"
                    aria-label={`Ver ${equip.name || "equipo"}`}
                  >
...
export default VisualizadorEquipmentTable;
