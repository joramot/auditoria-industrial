...
export const deletePedimento = async (equipmentId, pdfData, isOnline = true) => {
  try {
    console.log("🗑️ [deletePedimento] Iniciando eliminación:", pdfData.fileName);
    console.log("📋 [deletePedimento] equipmentId:", equipmentId);
    console.log("📋 [deletePedimento] pdfData:", pdfData);
    console.log("🌐 [deletePedimento] isOnline:", isOnline);

    if (isOnline) {
      // MODO ONLINE: Eliminar de Firebase Storage
      if (!pdfData.path) {
        throw new Error("No se encontró la ruta del PDF");
      }
    ...
    console.error("❌ [deletePedimento] Error:", error);
...
export const deleteFactura = async (equipmentId, pdfData, isOnline = true) => {
  try {
    console.log("🗑️ [deleteFactura] Iniciando eliminación:", pdfData.fileName);
    console.log("📋 [deleteFactura] equipmentId:", equipmentId);
    console.log("📋 [deleteFactura] pdfData:", pdfData);
...
export const deleteEquipmentImage = async (equipmentId, plantId, imageData, isOnline = true) => {
  try {
    console.log("🗑️ [deleteEquipmentImage] Iniciando eliminación:", imageData.name);
    console.log("📋 [deleteEquipmentImage] equipmentId:", equipmentId);
    console.log("📋 [deleteEquipmentImage] plantId:", plantId);
    ...
    console.error("❌ [deleteEquipmentImage] Error:", error);
    return { success: false, error: error.message };
  }
};
...
export const deletePlacaImage = async (equipmentId, plantId, imageData, isOnline = true) => {
  try {
    console.log("🗑️ [deletePlacaImage] Iniciando eliminación:", imageData.name);
    ...
    console.error("❌ [deletePlacaImage] Error:", error);
    return { success: false, error: error.message };
  }
};
...
export const deleteEquipmentComplete = async (equipmentId, plantId, isOnline = true) => {
  try {
    console.log("🗑️ [deleteEquipmentComplete] INICIANDO ELIMINACIÓN COMPLETA");
    console.log("📋 [deleteEquipmentComplete] equipmentId:", equipmentId);
    console.log("📋 [deleteEquipmentComplete] plantId:", plantId);
    console.log("🌐 [deleteEquipmentComplete] isOnline:", isOnline);
    ...
    if (isOnline) {
      ...
      if (!equipmentSnap.exists()) {
        console.error("❌ Equipo no encontrado en Firestore");
        return { success: false, error: "Equipo no encontrado" };
      }

      console.log("✅ Equipo encontrado en Firestore");
      let deletedImages = 0;
      let deletedPDFs = 0;
      ...
      for (const category of imageCategories) {
        ...
        try {
          ...
          console.log(`📁 Encontradas ${listResult.items.length} imágenes en ${category}`);
      ...
      for (const category of pdfCategories) {
        ...
        try {
          const listResult = await listAll(folderRef);
          console.log(`📁 Encontrados ${listResult.items.length} PDFs en ${category}`);
      ...
      // 4. Eliminar carpeta completa del equipo en Storage (limpieza adicional)
      try {
        ...
        console.warn("⚠️ No se pudo limpiar la carpeta completa:", err.message);
      }
      ...
      try {
        ...
        console.warn("⚠️ No se pudo actualizar el contador:", err.message);
      }
    ...
    console.error("❌ [deleteEquipmentComplete] ERROR CRÍTICO:", error);
...
export const deletePlantComplete = async (plantId, isOnline = true, onProgress = null) => {
  try {
    console.log("🗑️ [deletePlantComplete] INICIANDO ELIMINACIÓN COMPLETA DE PLANTA");
    console.log("📋 [deletePlantComplete] plantId:", plantId);
    ...
    if (isOnline) {
      ...
      // Primero guardamos todos los docs para evitar problemas con el forEach
      equipmentSnapshot.forEach((doc) => {
        equipmentDocs.push({ id: doc.id, data: doc.data() });
      });
      ...
      try {
        ...
        // Eliminar recursivamente todas las carpetas y archivos
        ...
        console.warn("⚠️ Error al limpiar carpeta de Storage:", err.message);
      }
    ...
    console.error("❌ [deletePlantComplete] ERROR CRÍTICO:", error);
    console.error("Stack trace:", error.stack);
    return { success: false, error: error.message };
  }
};
...
export const nukeEverything = async (confirmationText, onProgress = null) => {
  ...
  try {
    ...
    // 2. Eliminar cada planta
    let deletedPlants = 0;
    const totalStats = { equipment: 0, images: 0, pdfs: 0 };
    const plantDocs = [];
    
    plantsSnapshot.forEach((doc) => {
      plantDocs.push({ id: doc.id, data: doc.data() });
    });
...
export const getDeleteInfo = async (type, id) => {
  try {
    console.log(`📊 [getDeleteInfo] Obteniendo info para ${type}:`, id);
    ...
    if (type === "equipment") {
      ...
      const info = {
        name: data.name || data.equipmentName || "Sin nombre",
    ...
    if (type === "plant") {
      ...
      if (!plantSnap.exists()) {
        console.error("❌ Planta no encontrada:", id);
    ...
    console.error("❌ [getDeleteInfo] Error:", error);
    console.error("Stack trace:", error.stack);
    return { success: false, error: error.message };
  }
};
...
// ============================================================================
// EXPORTACIONES NOMBRADAS (sin default)
// ============================================================================

export {
  deletePedimento as deletePedimentoExport,
  deleteFactura as deleteFacturaExport,
  deleteEquipmentImage as deleteEquipmentImageExport,
  deletePlacaImage as deletePlacaImageExport,
  deleteEquipmentComplete as deleteEquipmentCompleteExport,
  deletePlantComplete as deletePlantCompleteExport,
  nukeLocalDatabase as nukeLocalDatabaseExport,
  nukeEverything as nukeEverythingExport,
  getDeleteInfo as getDeleteInfoExport
};
