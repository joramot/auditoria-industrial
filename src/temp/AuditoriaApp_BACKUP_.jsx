...
// NUEVO: Imports para limpieza de base de datos
import {
  findDuplicates,
  removeDuplicates,
  cleanEverything
...
const AuditoriaApp = () => {
  ...
  /**
   * Hook personalizado que obtiene el rol del usuario actual
   * Proporciona:
   * - userRole: Objeto completo con datos del rol
   * - isAdmin: Boolean - true si es administrador
   * - isSupervisor: Boolean - true si es supervisor  
   * - isAuditor: Boolean - true si es auditor
   * - loading: Boolean - true mientras carga el rol
   * - roleName: String - Nombre legible del ro
   */
  const { 
    userRole,           // Datos completos del rol
    isAdmin,            // ¿Es administrador?
    isSupervisor,       // ¿Es supervisor?
    isAuditor,          // ¿Es auditor?
    loading: roleLoading,  // ¿Está cargando el rol?
    roleName            // Nombre del rol para mostrar
  } = useRole();
...
const loadEquipment = async (plantId) => {
  ...
  const handleCancelEquipment = () => {
    console.log(" CANCELANDO EDICIÓN/CREACIÓN DE EQUIPO");
    ...
    try {
      ...
      if (isOffline) {
        ...
        if (result.success) {
          ...
          setSuccessMessage(
            `Planta "${newPlantData.name}" guardada y sincronizada con Firebase`
          );
          setShowSuccessMessage(true);
  ...
  const syncNow = async () => {
    if (isOffline) {
      alert(" No hay conexión. Conéctate a internet para sincronizar.");
      return;
    }
        ...
        <button
          onClick={async () => {
            try {
              ...
              console.error('â�Œ Error al cerrar sesión:', error);
            }
          }}
   ...
   * 1. Si está cargando autenticación → Loading
   * 2. Si no está autenticado → Login
   * 3. Si está cargando rol → Loading de Rol
   * 4. Si es Auditor → AuditorDashboard
   * 5. Si es Supervisor → Interfaz actual (captura)
   * 6. Si es Admin → Interfaz actual (puede ver todo)
   * 7. Si no tiene rol → Mensaje de error
   */
  
  // PASO 1: Loading de Autenticación
  if (isAuthLoading) {
    return (
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-800 mb-2">Cargando perfil...</h2>
          <p className="text-gray-600">Detectando rol de usuario</p>
          <div className="mt-4">
            <Loader className="w-8 h-8 text-blue-600 animate-spin mx-auto" />
          </div>
        </div>
  ...
  // PASO 4: Renderizado por Rol

  // 🎯 AUDITOR → Dashboard de Auditoría
  if (isAuditor) {
  ...
  // PASO 5: Sin Rol Asignado - Mensaje de Error
