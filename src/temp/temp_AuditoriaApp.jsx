...
const AuditoriaApp = () => {
  ...
  const handleSavePlant = async () => {
    ...
    if (result.success) {
      setSuccessMessage(
        `✓ Planta "${newPlantData.name}" guardada correctamente`
      );
  ...
  const handleSaveEquipment = async () => {
    ...
    try {
      ...
      if (isNewEquipment) {
        // ============================================
        // MODO CREACIÓN - Crear nuevo equipo
        // ============================================
        ...
        // ============================================
        console.log("✏️ Actualizando equipo existente:", selectedEquipment.id);

        equipmentId = selectedEquipment.id;

        const updateResult = await updateEquipment(equipmentId, equipmentData);
      ...
      for (const category of imageCategories) {
        ...
        if (images && images.length > 0) {
          ...
          for (const image of images) {
            // Solo subir imágenes NUEVAS (que tienen el flag isNew)
            if (image.isNew && image.file) {
              ...
              if (uploadResult.success) {
                ...
                console.warn(
                  `⚠️ No se pudo subir imagen: ${uploadResult.error}`
                );
              }
            } else if (image.url) {
      ...
      // ============================================
      // ACTUALIZAR EQUIPO CON URLs DE IMÁGENES
      // ============================================
      if (Object.keys(uploadedUrls).length > 0) {
        console.log("💾 Guardando URLs de imágenes...");
        await updateEquipment(equipmentId, { images: uploadedUrls });
      }

      // ============================================
      // MENSAJE DE ÉXITO
      // ============================================
      const action = isNewEquipment ? "guardado" : "actualizado";
      let message = `✓ Equipo "${formData.equipmentName}" ${action} correctamente`;

      if (totalImagesUploaded > 0) {
        message += ` con ${totalImagesUploaded} imagen(es)`;
      }
            ...
            >
              ...
              <div className="flex justify-between items-center mt-3 pt-3 border-t border-gray-100">
                ...
                <span className="text-xs text-gray-500">
                  Última auditoría: {plant.lastAudit}
                </span>
              </div>
            </div>
          ))}
                      ...
                      if (
                        ...
                        existingImages[category] = equip.images[category].map(
                          (img) => ({
                            ...
                            isNew: false, // Marcar como existente
                          })
                        );
                      }
                    });
        ...
        <div className="space-y-4">
          <div>
            ...
            <ImageUploader
              ...
              images={capturedImages.invoice}
              onImagesChange={(imgs) =>
                setCapturedImages({ ...capturedImages, invoice: imgs })
              }
              disabled={isLoading}
            />
          </div>
          ...
          <div>
            ...
            <ImageUploader
              ...
              images={capturedImages.customs}
              onImagesChange={(imgs) =>
                setCapturedImages({ ...capturedImages, customs: imgs })
              }
              disabled={isLoading}
            />
          </div>
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm text-blue-800">
            ...
            <p>
              ...
              información capturada en tiempo real.
            </p>
          </div>
        </div>
      </div>
