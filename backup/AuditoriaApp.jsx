...
const loadEquipment = async (plantId) => {
  ...
  const handleSaveEquipment = async () => {
    ...
    try {
      ...
      if (isOffline) {
        ...
        // ✅ CORRECCIÓN: Verificar si HAY URLs, no solo si se subieron archivos NUEVOS
        const hasImageUrls = Object.keys(uploadedUrls).some(cat => uploadedUrls[cat] && uploadedUrls[cat].length > 0);
        const hasPDFUrls = Object.keys(uploadedPDFUrls).some(cat => uploadedPDFUrls[cat] && uploadedPDFUrls[cat].length > 0);
        
        if (hasImageUrls || hasPDFUrls || totalImagesUploaded > 0 || totalPDFsUploaded > 0) {
          console.log(`📝 Actualizando equipo con URLs...`);
          console.log(`   📸 Imágenes subidas: ${totalImagesUploaded}`);
          console.log(`   📄 PDFs subidos: ${totalPDFsUploaded}`);
          console.log(`   📸 URLs de imágenes:`, JSON.stringify(uploadedUrls));
          console.log(`   📄 URLs de PDFs:`, JSON.stringify(uploadedPDFUrls));
          ...
          if (finalUpdate.success) {
            ...
            throw new Error(`Error al guardar URLs en Firestore: ${finalUpdate.error}`);
          }
        } else {
          console.log("ℹ️ No hay URLs de imágenes o PDFs para actualizar");
        }
    ...
    <div className="bg-blue-600 text-white p-4 shadow-lg sticky top-0 z-20">
      <div className="flex justify-between items-center mb-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold">Auditorí­a Industrial</h1>
        </div>
  ...
  const renderForm = () => (
    <div className="p-4 pb-24 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-4 mb-4">
        <h2 className="text-lg font-bold text-gray-800 mb-4">
          {selectedEquipment ? "âœï¸ Editar Equipo" : "➕ Nuevo Equipo"}
        </h2>
